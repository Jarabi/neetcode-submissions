...
class LinkedList:
    ...
    def remove(self, index: int) -> bool:
        ...
        if current and current_index == index:
            self.head = current.next    # Shift head pointer to the second node
            current = None              # Free memory/clean up pointer
            return True
        ...
        while current and current_index != index:
            prev = current
            current = current.next
            current_index += 1      # Increments counter to properly find the index
